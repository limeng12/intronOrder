...
#' Check if region1 is smaller than region2
#' 
#' @param region1 First genomic region
#' @param region2 Second genomic region
#' @return Logical vector
if_smaller_f <- function(region1, region2) {
  region1_start <- sapply(strsplit(region1, ":|-"), "[", 2)
  region2_start <- sapply(strsplit(region2, ":|-"), "[", 2)
